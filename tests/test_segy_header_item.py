...
from segytools.datatypes import DATA_SAMPLE_FORMAT_INT8, DATA_SAMPLE_FORMAT_INT16, DATA_SAMPLE_FORMAT_INT32, \
    DATA_SAMPLE_FORMAT_UINT8, DATA_SAMPLE_FORMAT_UINT16, DATA_SAMPLE_FORMAT_UINT32, \
    DATA_SAMPLE_FORMAT_FLOAT32, DATA_SAMPLE_FORMAT_FLOAT64, DATA_SAMPLE_FORMAT_IBM
from segytools.segy_header_item import SegyHeaderItem

def test_segy_header_item_value_initialization():
    expected = int(0)
    hdr_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT16, value=expected)
...
def test_segy_header_item_from_bytes_INT8_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_INT8, start_byte=0)
...
def test_segy_header_item_from_bytes_INT16_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_INT16, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_INT32_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_INT32, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_UINT8_ONE_LITTLE():
    expected = int(1)
    buf = struct.pack("<B", expected)
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_UINT8, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_UINT16_ONE_LITTLE():
    expected = int(1)
    buf = struct.pack("<H", expected)
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_UINT16, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_UINT32_ONE_LITTLE():
    expected = int(1)
    buf = struct.pack("<I", expected)
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_UINT32, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_FLOAT32_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_FLOAT32, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_FLOAT64_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_FLOAT64, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_from_bytes_IBM_POS_ONE_LITTLE():
    expected = float(1.0)
    buf = b'A\x10\x00\x00'
    header_item = SegyHeaderItem.from_bytes(buf=buf, endianess='<', name='example', sample_format=DATA_SAMPLE_FORMAT_IBM, start_byte=0)
    actual = header_item.value
    assert actual == expected
...
def test_segy_header_item_to_bytes_INT8_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT8, value=int(1))
...
def test_segy_header_item_to_bytes_INT16_ONE_LITTLE():
    ...
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT16, value=int(1))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected

def test_segy_header_item_to_bytes_INT32_ONE_LITTLE():
    expected = int(1).to_bytes(4, byteorder='little', signed=True)
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT32, value=int(1))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected

def test_segy_header_item_to_bytes_UINT8_ONE_LITTLE():
    expected = int(1).to_bytes(1, byteorder='little', signed=False)
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_UINT8, value=int(1))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected

def test_segy_header_item_to_bytes_UINT16_ONE_LITTLE():
    expected = int(1).to_bytes(2, byteorder='little', signed=False)
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_UINT16, value=int(1))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected

def test_segy_header_item_to_bytes_UINT32_ONE_LITTLE():
    expected = int(1).to_bytes(4, byteorder='little', signed=False)
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_UINT32, value=int(1))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected

def test_segy_header_item_to_bytes_FLOAT32_ONE_LITTLE():
    expected = struct.pack("<f", float(1.0))
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_FLOAT32, value=float(1.0))
    actual = header_item.to_bytes(endianess='<')
    assert actual == expected
...
def test_segy_header_item_has_mapping_dictionary_True():
    expected = True
    d = {1: 'has a mapping'}
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT16, value=int(1), map_dict=d)
    actual = header_item.has_mapping_dictionary()
    assert actual == expected
...
def test_segy_header_item_value_setter():
    expected = 1
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT16)
...
def test_segy_header_item_n_bytes_getter_INT8():
    expected = 1
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT8)
...
def test_segy_header_item_n_bytes_getter_INT32():
    expected = 4
    header_item = SegyHeaderItem(name='example', sample_format=DATA_SAMPLE_FORMAT_INT32)
    actual = header_item.n_bytes
    assert actual == expected
